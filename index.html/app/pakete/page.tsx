...
export const metadata: Metadata = {
  title: "Pakete — W&M Digital",
  description: "Starter, Professional und Premium — Festpreise für Automatisierung und Website bei W&M Digital.",
};
...
const ABOS = [
  {
    name: "Basis-Pflege",
    preis: "39 €/Monat",
    text: "Hosting, Sicherheitsupdates, automatische Backups, bis zu 2 kleine Textänderungen im Monat.",
  },
  {
    name: "Wachstum",
    preis: "89 €/Monat",
    text: "Alles aus Basis-Pflege + Pflege des Google-Unternehmenseintrags + 2 Social-Media-Beiträge/Monat + monatlicher Kurzreport.",
  },
];
      ...
      <section className="bg-papier2 py-16 md:py-20">
        <Container>
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2">
            {ABOS.map((abo) => (
              <div key={abo.name} className="rounded-sm border border-tinte/10 bg-papier p-6">
                <div className="flex items-baseline justify-between">
                  <h3 className="font-display text-lg font-semibold text-tinte">{abo.name}</h3>
                  <span className="font-mono text-sm text-messing">{abo.preis}</span>
                </div>
                <p className="mt-3 font-sans text-sm leading-relaxed text-tinte/65">{abo.text}</p>
              </div>
            ))}
          </div>
          <p className="mt-4 font-sans text-xs text-tinte/45">Monatlich kündbar.</p>
        </Container>
      </section>
      ...
      <section className="bg-papier py-16 md:py-20">
        <Container>
          ...
          <p className="mt-2 max-w-xl font-sans text-sm text-tinte/60">
            Diese Kosten fließen nicht an W&amp;M Digital, sondern direkt an
